			...
			<view class="content-body">
				...
				<view class="footer flex-center">
					...
					<template v-else>
						...
						<button v-if="installed && isWGT " class="content-button" style="border: none;color: #fff;"
							plain @click="restart">
							安装完毕，点击重启
						</button>
					</template>
	...
	export default {
		...
		onLoad({
			...
			if (!local_storage_key) {
				...
				uni.showToast({
					title: 'local_storage_key为空，请检查后重试',
					duration: 1000
				});
				uni.navigateBack()
				return;
			...
			// uni.showToast({
			// 	title: JSON.stringify(localPackageInfo),
			// 	duration: 1000
			// });
		...
		methods: {
			...
			downloadPackage() {
				...
				// } else {
				// 	let urlStr = encodeURI(this.url)
				// 	var dtask = plus.downloader.createDownload(urlStr, {}, function(
				// 		d,
				// 		status
				// 	) {
				// 		// 下载完成，打开文件
				...
				// 	// plus.runtime.openURL(urlStr, err => {
				// 	// 	uni.showToast({
				// 	// 		title: urlStr,
				// 	// 		icon: 'none',
				// 	// 		duration: 1000
			...
			installPackage() {
				...
				plus.runtime.install(this.tempFilePath, {
					...
					if (this.isWGT) {
						...
						// uni.showLoading({
						// 	icon: 'none',
						// 	title: this.tempFilePath,
						// 	duration: 1000
						// })
						plus.runtime.openFile(this.tempFilePath)
						// const localFilePathRecord = uni.getStorageSync(localFilePathKey)
